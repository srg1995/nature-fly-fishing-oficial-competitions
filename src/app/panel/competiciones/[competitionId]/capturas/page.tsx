...
export default function CapturasPage() {
  ...
  const { data: participantes = [] } = useParticipantes(competition.id);
  const { data: stats, isLoading: statsLoading } = useStats(competition.id);
  const createCaptura = useCreateCaptura(competition.id);
  const deleteCaptura = useDeleteCaptura(competition.id);
