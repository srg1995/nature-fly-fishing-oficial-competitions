...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const participante = await getParticipanteById(competitionId, id);
    if (!participante) {
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const existing = await getParticipanteById(competitionId, id);
    if (!existing) {
      return NextResponse.json(
        { error: "Participante no encontrado" },
        { status: 404 }
    ...
    const updated = await updateParticipante(competitionId, id, parsed.data);
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    await deleteParticipante(competitionId, id);
